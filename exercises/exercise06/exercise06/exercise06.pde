...
// Ball objects are for making the video image update similar to animation
Ball[] balls;
...
void setup() {
  ...
  for (int i = 0; i < bouncers.length; i++) {
    ...
    bouncers[i] = new Bouncer(random(0, width), random(0, height), random(-10, 10), random(-10, 10), random(10, 25), color(random(255)));
  }
  // Checking through each constructor of the ball object in the array list.
...
// video.read is outseide the loop draw make ball objects continuesly draw on top of eachther.
...
void draw() {
  ...
  // Displaying and updating the ball's position on the screen  
  for (int i = 0; i < balls.length; i++) {  
    balls[i].move();
    balls[i].display();
  }
